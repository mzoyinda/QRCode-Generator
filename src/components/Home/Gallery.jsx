...
const Gallery = () => {
  ...
  const { isLoading, error } = useSelector((state) => state.placeholders);
    ...
    <section>
      <div className="md:mx-14">
        ...
        {isLoading || error ? (
          ...
          <h3 className="pt-20 text-[25px]"> {error ? "Unable to fetch album at the moment!" : "Fetching Album..."}</h3>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
            {filteredAlbum.slice(0, 10).map((photo, index) => (
              <div
                key={index}
